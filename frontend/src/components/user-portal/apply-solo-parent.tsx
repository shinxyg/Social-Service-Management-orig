...
import { Check, ClipboardList, ChevronRight, IdCard, ShieldAlert, ImagePlus, X } from "lucide-react"
import { PageHeader } from "../ui/shared"
import { Field, inputCls } from "../ui/form-ui"

type ApplicationType = "Solo Parent ID" | "Child Welfare Case"

const TYPE_ITEMS: { value: ApplicationType; label: string; desc: string; icon: typeof IdCard }[] = [
  {
    value: "Solo Parent ID",
    label: "Solo Parent ID",
    desc: "Register under the Solo Parents Welfare Act for benefits access",
    icon: IdCard,
  },
  {
    value: "Child Welfare Case",
    label: "Child Welfare Case",
    desc: "Report or request assistance for a child protection concern",
    icon: ShieldAlert,
  },
]

function generateReference(type: ApplicationType) {
  const prefix = type === "Solo Parent ID" ? "SP" : "CW"
...
export default function ApplySoloParent() {
  ...
  const [appType, setAppType] = useState<ApplicationType>("Solo Parent ID")
  ...
  const isChildWelfare = appType === "Child Welfare Case"
  const canSubmit = name.trim() && address.trim() && contact.trim() && dependents.trim() && proofOfStatus.trim()

  const handleSubmit = () => {
    if (!canSubmit) return
    setReference(generateReference(appType))
    setSubmitted(true)
  }

  const handleSelectType = (value: ApplicationType) => {
    setAppType(value)
    setView("form")
  }
      ...
      <div className="p-4 md:p-6 max-w-xl mx-auto">
        <div className="bg-card border border-border rounded-2xl p-8 shadow-soft flex flex-col items-center text-center gap-3">
          ...
          <p className="text-sm text-muted-foreground max-w-sm">
            Thank you, {name}. Your {appType.toLowerCase()} request has been received and is now pending review
            by a social worker.
          </p>
    ...
    return (
      <div className="p-4 md:p-6 max-w-4xl mx-auto">
        ...
        <div className="mt-6 bg-muted/50 border border-border rounded-2xl p-5 text-sm text-muted-foreground flex items-start gap-2.5">
          ...
          Pick a type above to start your application. Bring a valid ID, proof of status (Barangay Certificate /
          death certificate / court order), and birth certificate(s) of your child/children when a social worker
          asks you to visit for verification.
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-2xl mx-auto">
      ...
      <PageHeader
        title={isChildWelfare ? "Report a Child Welfare Case" : "Apply for Solo Parent ID"}
        desc="Fill out this form and a social worker will review your submission and contact you for the next steps."
      />
      ...
      <div className="bg-card border border-border rounded-2xl p-6 shadow-soft space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Application type" full>
          ...
          <Field label={isChildWelfare ? "Details of the case" : "Proof of status"} full>
            <textarea
              value={proofOfStatus}
              ...
              placeholder={
                isChildWelfare
                  ? "Briefly describe the concern involving the child..."
                  : "Briefly describe your basis for solo parent status (e.g. widowed, separated, OFW spouse)..."
              }
            />
          </Field>
