...
function BeneficiaryProfileModal({
  ...
  const [showEditForm, setShowEditForm] = useState(b.verificationStatus !== "verified")
        ...
        <div className="px-6 py-6 overflow-y-auto space-y-6 flex-1">
          ...
          {tab === "verification" && (
            <div className="space-y-4">
              ...
              {b.verificationStatus === "verified" && !showEditForm ? (
                <div className="flex justify-end pt-1">
                  <button
                    type="button"
                    onClick={() => setShowEditForm(true)}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 text-xs font-semibold text-slate-700 bg-slate-50 hover:bg-slate-100 hover:text-slate-900 transition-colors shadow-2xs"
                  >
                    <Edit3 className="h-3.5 w-3.5 text-slate-500" />
                    Change Status / Re-evaluate
                  </button>
                </div>
              ) : (
